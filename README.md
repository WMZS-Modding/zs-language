...
- Nouns: Special noun type. Use "itself" (no need to use `{}`) to make the prefab itself active, use `<>` (*Function noun*) and `()` (*Script noun*) for tag or prefab, function or local to call an external prefab. `/||\` is the Tilemap noun, you must write correct tilemap name, otherwise, your script won't work
